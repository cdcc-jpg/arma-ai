    ...
    [_lead, _wing, _spawnedTargets, _targetPositions] spawn {
        ...
        for "_i" from 0 to ((count _targets) - 1) do {
            ...
            if (!alive _lead && {!alive _wing}) exitWith {
                ...
                diag_log format ["[AGIA MARINA - SEMANTIQUE] ECHEC ! Le binome BLUFOR a ete elimine a la cible %1 (%2/5 neutralisees).", _targetNum, _i];
            };

            // If target was bypassed or suppressed by close-quarters domination, ensure clean elimination
            ...
            diag_log format ["[AGIA MARINA - SEMANTIQUE] %1 neutralisee ! (%2/5 terminees)", _callsign, _targetNum];
            sleep 0.4;
        };
