/*
    ...
    Mission: Georgetown: Vanilla AI Trinome Benchmark (Tanoa)
    File: init.sqf
    ...
    Description:
        ...
        by the native Bohemia Interactive Vanilla AI engine.
        Strict 1-to-1 parity with the TICO Semantic benchmark mission.
...
diag_log "[AAI Benchmark Georgetown] Initializing Vanilla AI Trinome Benchmark on Tanoa...";
...
AAI_fnc_resetBenchmarkTrial = {
    ...
    // =========================================================================
    // SPAWN BLUFOR TRINOME (Vanilla AI - Full Native Routines Enabled)
    // =========================================================================
    private _grpBlufor = createGroup [west, true];
    _grpBlufor enableAttack true;
    ...
    _wing1 setVariable ["AAI_Callsign", "VANILLA APPUI [AR]"];
    ...
    _wing2 setVariable ["AAI_Callsign", "VANILLA TIREUR [PRECISION]"];
    ...
    [_lead, _wing1, _wing2, _spawnedTargets, _targetPositions] spawn {
        ...
        for "_i" from 0 to ((count _targets) - 1) do {
            ...
            if (alive _wing1) then {
                _wing1 setVariable ["AAI_TacticalObjective", _currentPos];
                _wing1 setVariable ["AAI_CurrentTargetIndex", _targetNum];
                if (alive _lead) then { _wing1 doFollow _lead; } else { _wing1 doMove _currentPos; };
            };

            if (alive _wing2) then {
                _wing2 setVariable ["AAI_TacticalObjective", _currentPos];
                _wing2 setVariable ["AAI_CurrentTargetIndex", _targetNum];
                if (alive _lead) then { _wing2 doFollow _lead; } else { _wing2 doMove _currentPos; };
            };

            systemChat format ["[GEORGETOWN - VANILLA] Trinome Vanilla en sprint vers %1...", _callsign];

            // Wait until target is killed, or dominated in close quarters (< 6.0m), or all BLUFOR die
            waitUntil {
            ...
            if (!alive _lead && {!alive _wing1} && {!alive _wing2}) exitWith {
                systemChat format ["[GEORGETOWN - VANILLA] ECHEC ! Le trinome Vanilla a ete elimine a la cible %1 (%2/8 neutralisees).", _targetNum, _i];
                diag_log format ["[GEORGETOWN - VANILLA] ECHEC ! Le trinome Vanilla a ete elimine a la cible %1 (%2/8 neutralisees).", _targetNum, _i];
            };
            ...
            systemChat format ["[GEORGETOWN - VANILLA] %1 neutralisee ! (%2/8 terminees)", _callsign, _targetNum];
            diag_log format ["[GEORGETOWN - VANILLA] %1 neutralisee ! (%2/8 terminees)", _callsign, _targetNum];
            sleep 0.4;
        };
        ...
        if (alive _lead || alive _wing1 || alive _wing2) then {
            ...
            systemChat format ["[GEORGETOWN - VANILLA] VICTOIRE DU TRINOME VANILLA ! (Lead: %1/250 HP, AR: %2/250 HP, Marks: %3/250 HP)", _hpLead, _hpWing1, _hpWing2];
        };
    };
    ...
    [_lead, _wing1, _wing2, _spawnedTargets, _targetPositions] spawn {
        params ["_lead", "_wing1", "_wing2", "_targets", "_positions"];
        private _t0 = time;
        diag_log format ["[BENCHMARK_TRACK_INIT] mission=VANILLA;blufor=[%1,%2,%3];targets=%4",
        ...
        while {({alive _x} count [_lead, _wing1, _wing2] > 0) && {({alive _x} count _targets > 0)}} do {
            ...
            // Log BLUFOR units
            {
                if (!isNull _x && {alive _x}) then {
                    ...
                    diag_log format ["[TRACK_BLUFOR] mode=VANILLA;t=%1;id=%2;role=%3;x=%4;y=%5;dir=%6;spd=%7;stc=%8;hp=%9;state=%10;tgt=%11",
            ...
            diag_log format ["[TRACK_OPFOR] mode=VANILLA;t=%1;alive=%2", _elapsed, _opforAlive];

            sleep 0.5;
        };

        private _finalElapsed = round ((time - _t0) * 10) / 10;
        diag_log format ["[BENCHMARK_TRACK_END] mode=VANILLA;duration=%1;bluforSurvivors=%2;targetsNeutralized=%3",
            _finalElapsed,
    ...
    systemChat "[BENCHMARK GEORGETOWN : VANILLA TRINOME] Epreuve prete (8 cibles). F1 Reset | F2 Camera (Lead/AR/Marks/Spectateur) | Y/Z Zeus";
};
    ...
    switch (_nextMode) do {
        case 1: {
            if (!isNull _lead && {alive _lead}) then {
                ...
                systemChat "[CAMERA] Mode: YEUX DU POINTMAN VANILLA (FPV Lead).";
            } else {
        ...
        case 2: {
            if (!isNull _wing1 && {alive _wing1}) then {
                ...
                systemChat "[CAMERA] Mode: YEUX DU MITRAILLEUR VANILLA (FPV Appui AR).";
            } else {
                call AAI_fnc_cycleCameraView;
            };
        };
        case 3: {
            if (!isNull _wing2 && {alive _wing2}) then {
                ...
                systemChat "[CAMERA] Mode: YEUX DU TIREUR D'ELITE VANILLA (FPV Marksman).";
            } else {
                call AAI_fnc_cycleCameraView;
            };
        };
...
// ============================================================================
// 6. Real-Time Benchmark Telemetry HUD Loop (Vanilla Trinome Telemetry)
// ============================================================================
[] spawn {
    while {true} do {
        sleep 0.25;
        ...
        private _camName = switch (_camMode) do {
            case 1: { "Pointman Vanilla (FPV Lead)" };
            case 2: { "Autorifleman Vanilla (FPV Appui)" };
            case 3: { "Marksman Vanilla (FPV Precision)" };
        ...
        hintSilent parseText format [
            "<t color='#ffaa33' size='1.2' font='PuristaBold'>[BENCHMARK GEORGETOWN : VANILLA TRINOME]</t><br/>" +
            "<t color='#aaaaaa' size='0.85'>Vue Active :</t> <t color='#ffff00'>%1</t><br/><br/>" +
            "<t color='#ffffff' font='PuristaBold'>--- POINTMAN (LEAD) ---</t><br/>" +
            "Sante : <t color='%2'>%3 / 250 HP</t> | Moteur : <t color='#ffaa33'>Arma 3 Vanilla FSM</t><br/><br/>" +
            "<t color='#ffffff' font='PuristaBold'>--- APPUI LOURD (AR) ---</t><br/>" +
            "Sante : <t color='%4'>%5 / 250 HP</t> | Moteur : <t color='#ffaa33'>Arma 3 Vanilla FSM</t><br/><br/>" +
            "<t color='#ffffff' font='PuristaBold'>--- PRECISION (MARKSMAN) ---</t><br/>" +
            "Sante : <t color='%6'>%7 / 250 HP</t> | Moteur : <t color='#ffaa33'>Arma 3 Vanilla FSM</t><br/><br/>" +
            "Progression : <t color='#ffffff'>%8 m / 325 m</t><br/>" +
            "Score Cibles : <t color='#ffff00'>%9 / 8 neutralisees</t><br/>" +
            ...
            _camName,
            _hColorLead, _hpLead,
            _hColorWing1, _hpWing1,
            _hColorWing2, _hpWing2,
...
diag_log "[AAI Benchmark Georgetown] Vanilla AI Trinome Mission Initialized Successfully.";
