...
private _fnc_classifyObstacle = {
    ...
    // 2. Concrete, stone, metal walls, sandbags, fortifications, containers
    if (_typeName find "Wall" != -1 || 
        ...
        {_typeName find "Concrete" != -1}) exitWith {
        "SolidCover"
    };
    ...
    // 4. Buildings & houses
    if (_obj isKindOf "House" || {_obj isKindOf "Building"}) exitWith {
        "SolidCover"
    };
...

{
    ...
    // Filter out invalid/ignored objects, living units, dead bodies, projectiles
    if (!isNull _obj && 
        {!(_obj in _ignored)} && 
        {!(_obj isKindOf "Man")} && 
