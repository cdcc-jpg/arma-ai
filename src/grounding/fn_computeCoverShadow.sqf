...
// Optimal Primary Cover Point: Offset from rear face along threat direction by standoff distance
private _coverPoint = _backCenter vectorAdd (_threatDir2D vectorMultiply _standoffDist);
// Ensure cover point is grounded on terrain ATL
...
// Physical Occlusion Raycast Check:
...
// Higher score for solid cover, greater obstacle height, larger width, and verified occlusion
private _heightFactor = (_obsHeight / 2.0) min 1.0;
private _widthFactor = (_obsWidth / 3.0) min 1.0;
private _solidFactor = if (_classification == "SolidCover") then { 1.0 } else { 0.45 };
private _occlusionFactor = if (_isOccluded) then { 1.0 } else { 0.1 };

private _qualityScore = (_heightFactor * 0.35) + (_widthFactor * 0.25) + (_solidFactor * 0.2) + (_occlusionFactor * 0.2);
